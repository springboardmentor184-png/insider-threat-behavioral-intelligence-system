...
def analyze_employee(employee_id: str, db: Session = Depends(get_db)):
    logs = db.query(models.ActivityLog).filter(models.ActivityLog.employee == employee_id).all()

    if not logs:
        raise HTTPException(status_code=404, detail="No activity found for this employee")
...
def get_all_risk_scores(db: Session = Depends(get_db)):
    employees = db.query(models.UserProfile).limit(3).all()
    ...
    results = (
        db.query(
            ...
            func.sum(
                case(
                    (models.ActivityLog.activity.in_(["USB Connect", "USB Disconnect"]), 1),
                    else_=0
                )
            ).label("usb_activity"),
            func.sum(
    ...
    for row in results:
        ...
        raw_data.append({
            ...
            "unusual_login_ratio": float(row.unusual_login) /float (row.total_logs),
            "usb_ratio": float(row.usb_activity) / float (row.total_logs),
            "email_ratio": float (row.email_count) / float (row.total_logs),
            "web_ratio": float (row.web_access_count) /float (row.total_logs)
        })

    df = pd.DataFrame(raw_data)

    feature_columns = ["unusual_login_ratio", "usb_ratio", "email_ratio", "web_ratio"]

    global _anomaly_model_cache, _trained_model_cache
    if _anomaly_model_cache is None:
        _anomaly_model_cache = AnomalyDetection()
        _trained_model_cache = _anomaly_model_cache.train(df[feature_columns])

    anomaly_scores = _anomaly_model_cache.get_anomaly_score(
        _trained_model_cache, df[feature_columns], df["employee"]
    )

    score_map = {r["employee_id"]: r["risk_score"] for r in anomaly_scores}
    ...
    results_sorted = sorted(final_results, key=lambda x: x["risk_score"], reverse=True)
    return results_sorted


@router.get("/risk_summary")
def get_risk_summary(db: Session = Depends(get_db)):
    all_scores = get_all_risk_scores(db)
    ...
    return [{"category": k, "count": v} for k, v in summary.items()]

@router.get("/anomaly_report")
def get_anomaly_report(db: Session = Depends(get_db)):
    all_scores = get_all_risk_scores(db)
    ...
    total_flagged = sum(
        severity_breakdown[s] for s in ["Medium", "High", "Critical"]
    )
