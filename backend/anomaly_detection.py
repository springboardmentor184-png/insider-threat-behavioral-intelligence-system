...
from models import ActivityLog, BehavioralBaseline, Anomaly, Alert
...
def run_anomaly_detection(db):
    ...
    for log in logon_logs:
        ...
        if abs(z_score) >= Z_SCORE_THRESHOLD:
            ...
            db.add(anomaly)
            db.flush()  # obtain the anomaly ID before creating an alert
            if anomaly.severity in {"high", "critical"}:
                db.add(Alert(
                    employee_id=anomaly.employee_id,
                    alert_type="behavioral_anomaly",
                    severity=anomaly.severity,
                    title=f"{anomaly.severity.title()} unusual login time",
