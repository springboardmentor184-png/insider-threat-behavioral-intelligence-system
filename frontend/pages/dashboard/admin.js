...
export default function AdminDashboard() {
  ...
  const changeRole = async (userId, role) => {
    ...
    try {
      const updated = await apiFetch(`/analytics/admin/users/${userId}/role`, { method: "PATCH", body: JSON.stringify({ role }) });
    ...
    <Layout title="Platform overview" subtitle="System health, data coverage, and console user management">
      {loading ? (
        ...
        <>
          ...
          <UserManagementTable users={users} onRoleChange={changeRole} updatingUserId={updatingUserId} />
        </>
      )}
    </Layout>
  );
