...
def calculate_risk(row):
    ...
    if row["offhour_percentage"] > 70:
        score += 10
        reasons.append(f"Very high off-hour activity ({row['offhour_percentage']:.1f}%)")
    elif row["offhour_percentage"] > 50:
        score += 8
        reasons.append(f"High off-hour activity ({row['offhour_percentage']:.1f}%)")
    elif row["offhour_percentage"] > 30:
        score += 5
        reasons.append(f"Moderate off-hour activity ({row['offhour_percentage']:.1f}%)")
    ...
    if row["unique_pcs"] > 1000:
        score += 10
        reasons.append(f"Used {int(row['unique_pcs'])} different PCs")
    elif row["unique_pcs"] > 500:
        score += 8
        reasons.append(f"Used {int(row['unique_pcs'])} different PCs")
    elif row["unique_pcs"] > 100:
    ...
    if row["device_switches"] > 1000:
        score += 10
        reasons.append(f"{int(row['device_switches'])} device switches")
    elif row["device_switches"] > 500:
        score += 8
        reasons.append(f"{int(row['device_switches'])} device switches")
    elif row["device_switches"] > 100:
    ...
    if row["avg_session_hours"] > 10:
        score += 5
        reasons.append(f"Average session {row['avg_session_hours']:.1f} hours")
    elif row["avg_session_hours"] > 8:
        score += 3
        reasons.append(f"Average session {row['avg_session_hours']:.1f} hours")
    ...
    if row["total_logins"] > 2000:
        score += 5
        reasons.append(f"{int(row['total_logins'])} total logins")
    elif row["total_logins"] > 1000:
        score += 3
        reasons.append(f"{int(row['total_logins'])} total logins")
...
] = baseline.apply(calculate_risk, axis=1)

baseline = baseline.sort_values(by="risk_score", ascending=False)
...
print(baseline["risk_level"].value_counts())
